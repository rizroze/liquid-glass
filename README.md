...
- **`backdrop-filter: url()` is Chromium-only.** Not a webkit thing, Safari doesn't support it either
- **SVG filter must be in the same document.** Shadow DOM won't work
- **Large elements (>800px wide)** may cause GPU jank, so reduce `scale` for large surfaces
...
- **`opacity < 1` on a parent** creates a stacking context that can interfere, so test your specific layout
